...
std::string VirtualMachine::get_response(const std::string &command) {
    ...
    if (command.starts_with('e')) {
        ...
        send_measurements(command.substr(1), 1000 / ch1_frequency);
        return "";
    }
    if (command.starts_with('h')) {
        SPDLOG_INFO("Reading measurements from channel {}", 2);
        send_measurements(command.substr(1), 1000);
        return "";
    }
...
/// @param sleep_ms Sleep time in milliseconds between sending each measurement
void VirtualMachine::send_measurements(const std::string &num_str, const int sleep_ms) {
    ...
    const auto stop_line = std::make_shared<boost::asio::streambuf>();
    boost::asio::async_read_until(get_port(), *stop_line, "\n",
                                  [&stop_requested, stop_line](const boost::system::error_code &ec, std::size_t) {
                                      if (ec) return; // cancelled once streaming ends, nothing to do
                                      std::istream is(stop_line.get());
                                      std::string line;
                                      std::getline(is, line);
                                      if (!line.empty() && line.front() == 'c') stop_requested = true;
                                  });
    get_io().restart();
    while (!stop_requested) {
        ...
        write_raw_to_serial(measurement);
        if (i == number_of_measurements || stop_requested) {
            SPDLOG_INFO("Measurement stream completed: {} measurements sent", i);
            break;
        }
        // Wait between measurements while still polling for the stop command to arrive
        const auto next_send = std::chrono::steady_clock::now() + std::chrono::milliseconds(sleep_ms);
        while (!stop_requested && std::chrono::steady_clock::now() < next_send) {
            get_io().poll_one();
